...
extra_data_test = csr_matrix(test_cleaned[X_cols])
X_test = hstack([X_words_test, extra_data_test]).tocsr()
del X_words_test
#del extra_data
gc.collect()
...
X_train = hstack([X_words_inverse, extra_data]).tocsr()
extra_data = csr_matrix(test_cleaned[X_cols])
X_test = hstack([X_words_test_inverse, extra_data_test]).tocsr()

print('\t- Fitting (Inverse)...')
_fit(X_train, y_train, verbose, EARLY_STOPPING_ROUNDS)

print('\t- Re-Fitting with all data (Inverse)...')
_re_fit(X_train, y_train, verbose, int(lgb_model.best_iteration_*1.3))
