...
class r_data(object):
    ...
    def __init__(self, MAX_LEN, vec='hash'):
        ...
        if vec == 'hash':
            ...
            self.vectorizer = TfidfVectorizer(ngram_range=(1,2),
               ...
               smooth_idf=1, sublinear_tf=1, max_features=self.MAX_LEN)
    ...
    def clean_text(self, df):
        ...
        df["num_bad_words"] = df["comment_text"].apply(lambda x: len([word for word in x.split() if word.lower() in self.arrBad]))
        df["num_good_words"] = df["comment_text"].apply(lambda x: len([word for word in x.split() if word.lower() in self.arrGood]))
...
#    .sample(200000, random_state=42).reset_index(drop=True)
    print('\t- Cleaning data...')
    train = data.clean_text(train)
    print('\t- Vectorizer...')
    X_words = data.vector(train['comment_text'].values)
...
#    X_train = data.final_X(X_words, train[X_cols].values)
#    del X_words
#    gc.collect()
    ...
    print('\t- Vectorizer...')
    X_words = data.vector(test['comment_text'].values, train=False)
    ...
    X_test = hstack([X_words, extra_data]).tocsr()
